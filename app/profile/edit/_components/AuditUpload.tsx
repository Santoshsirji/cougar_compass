...
interface AuditUploadProps {
    currentAuditFilePath: string | null | undefined;
}

export function AuditUpload({ currentAuditFilePath }: AuditUploadProps) {
    const [selectedFile, setSelectedFile] = useState<File | null>(null);
    const [isUploading, startUploadTransition] = useTransition();
    const { toast } = useToast();
    const fileInputRef = useRef<HTMLInputElement>(null); // Ref to reset file input
    const [currentPath, setCurrentPath] = useState(currentAuditFilePath); // Local state to update UI immediately

    const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        const file = event.target.files?.[0];
        if (file) {
             // Basic client-side validation (optional, server validates anyway)
    ...
    const handleUpload = () => {
        ...
        startUploadTransition(async () => {
            ...
            if (result.success) {
                ...
                setCurrentPath(result.filePath); // Update displayed path
            } else {
        ...
        <div className="space-y-4 rounded-md border p-4">
             ...
             {currentPath ? (
                <div className="text-sm text-muted-foreground flex items-center space-x-2">
                    ...
                    <Link href={currentPath} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline inline-flex items-center">
                         {currentPath.split('/').pop()} {/* Show filename */} 
